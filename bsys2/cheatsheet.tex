...
\begin{description}
  ...
  \item[Rechnen mit Zweierpotenzen] $1 = 2^0, \ldots 512 = 2^9$; $\si{\kilo\byte} = 2^{10}, \si{\mega\byte}
    = 2^{20}, \si{\giga\byte} = 2^{30}, \ldots$ $\Rightarrow$ $\SI{32}{\kilo\byte} = 2^{15}$
\end{description}
...
($\rightarrow$ 12 offset bits). Hauptspeicher wird in \emph{page frames}
...
\begin{itemize}
  \item \SI{4}{\kilo\byte} page size ($2^{12} \si{\byte}$)
  \item \SI{4}{\giga\byte} RAM ($2^{32} \si{\byte}$) $\rightarrow 2^{20}$ (1M) page frames $\rightarrow \SI{20}{\bit}$ page nr
...
\item[Port mapped via Speicherbus] RAM-Bus hat zusätzliches selector bit, CPU
...
\begin{description}
  ...
  \item[FAT] Enthält Eintrag pro Cluster (erster Cluster einer Datei). Der
...
GetMessage get_message = (GetMessage)dlsym(lib, "get_message");
